...
(much faster than using a C<SearchForward()>, C<Delete()> loop.)
...
=item ( $PValue, $Value, $FoundKey ) = SearchForward( $Judy, $Key )
...
find the first key in the array.) SearchForward() is typically used to
I<begin> a sorted-order scan of the valid C<$Key>s in a JudySL array.

    my ( undef, $Value, $Key ) = SearchForward( $Judy, '' );
    while ( $Key ) {
        print "$Key=$Value\n";
        ( undef, $Value, $Key ) = ContinueForward( $Judy, $Key );
    }
...
=item ( $PValue, $Value, $FoundKey ) = ContinueForward( $Judy, $Key )
...
in the array.) C<ContinueForward()> is typically used to I<continue> a
sorted-order scan of the valid C<$Key>s in a JudySL array.

=item ( $PValue, $Value, $FoundKey ) = SearchBackward( $Judy, $Key )
...
C<0xff> bytes.)  C<SearchBackward()> is typically used to C<begin> a
reverse-sorted-order scan of the valid keys in a JudySL array.

    my ( undef, $Value, $Key ) = SearchBackward( $Judy, '' );
    while ( $Key ) {
        print "$Key=$Value\n";
        ( undef, $Value, $Key ) = ContinueBackward( $Judy, $Key );
    }

=item ( $PValue, $Value, $FoundKey ) = ContinueBackward( $Judy, $Key )

Search (inclusive) for the first C<$Key> present that is less than the
passed C<$Key> string. C<ContinueBackward()> is typically used to C<continue> a
reverse-sorted-order scan of the valid keys in a JudySL array.
